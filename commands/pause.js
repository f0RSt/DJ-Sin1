...
module.exports = {
  name: "pause",
  description: "Ставит бота на паузу",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (player.paused)
      return client.sendTime(
        message.channel,
        "❌ | **Ты че? Музыка уже поставлена ​​на паузу!**"
      );
    player.pause(true);
    let embed = new MessageEmbed()
      .setAuthor(`Приостановлено!`, client.botconfig.IconURL)
      .setColor(client.botconfig.EmbedColor)
      .setDescription(`Напишите \`${GuildDB.prefix}resume\` чтобы продолжить!`);
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (player.paused)
        return client.sendTime(interaction, "Ты че? Музыка уже поставлена ​​на паузу!");
      player.pause(true);
