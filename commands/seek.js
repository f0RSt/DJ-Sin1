...
module.exports = {
  name: "seek",
  description: "Ищите позицию в песне",
  usage: "<time s/m/h>",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SeekTo)
      return client.sendTime(
        message.channel,
        `**Применение - **\`${GuildDB.prefix}искать <number s/m/h>\` \n**Пример - **\`${GuildDB.prefix}искать 2m 10s\``
      );
        ...
        options: [
            {
                ...
                *
                run: async (client, interaction, args, { GuildDB }) => {
                    ...
                    if (guild.me.voice.channel && !guild.me.voice.channel.equals(member.voice.channel)) return client.sendTime(interaction, "❌ | **Вы должны быть на том же голосовом канале, что и я, чтобы использовать эту команду!**");
                    if (!player) return client.sendTime(interaction, "❌ | **Сейчас ничего не воспроизводится...**");
                    if (!player.queue.current.isSeekable) return client.sendTime(interaction, "❌ | **Я не могу найти эту песню!**");
