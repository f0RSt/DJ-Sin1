...
module.exports = {
  name: "queue",
  description: "Показывает все песни, которые в данный момент находятся в очереди",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue || !player.queue.length || player.queue === 0) {
      let QueueEmbed = new MessageEmbed()
        ...
        .addField("Запрошенно", `${player.queue.current.requester}`, true)
        .addField(
          "Продолжительность",
          `${
            client.ProgressBar(
