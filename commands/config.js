...
`);
    ...
    let emoji = await ConfigMessage.awaitReactions(
      ...
      client.sendTime(
        message.channel,
        "❌ | **Вы слишком долго не отвечали. Если вы хотите изменить настройки, запустите команду еще раз!**"
      );
    ...
    if (em._emoji.name === "1️⃣") {
      ...
      client.sendTime(
        message.channel,
        `Guild prefix успешно сохранен как \`${prefix}\``
      );
    } else {
      await client.sendTime(
        message.channel,
        "Пожалуйста, укажите роль, которую вы хотите чтобы `DJ's` имел."
      );
      ...
      if (!role.mentions.roles.first())
        return client.sendTime(
          message.channel,
          "Пожалуйста, укажите роль, которую вы хотите только для DJ's."
        );
  ...
  SlashCommand: {
    options: [
      {
        name: "prefix",
        description: "Проверяет префикс бота",
        type: 1,
        required: false,
        options: [
          {
